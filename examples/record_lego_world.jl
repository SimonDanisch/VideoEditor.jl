...
const PROJECT  = joinpath(MEDIA, "lego_world.videoedit")
const RAW_MP4  = joinpath(tempdir(), "lego_world_raw.mp4")
const OUT_MP4  = joinpath(MEDIA, "lego_world_walkthrough.mp4")
const EXPORTED = joinpath(MEDIA, "lego_world_export.mp4")
...
# `gpupreview = false`. The GPU preview tier hands the composited canvas to
# GLMakie through a shared Vulkan image, and its autodetect runs a render from an
# async task while the main one is rendering too — with TWO RayMakie scene clips
...
# window. Pay it here.
let t0 = time()
    # TWO scene clips now — a LEGO world on track 1 and the figure over it on
    # track 2 — so the warm-up pays for both.
    for _ in 1:4
    ...
    VE.showplayhead!(player)
    sleep(1.0)
    # REBUILD THE SCENE CARD, now that the scene exists.
    #
    # A scene card's sections come from `sceneattributes`, which reads the LIVE
    # scene — and the card is built when the clip is first shown, which happens
    # before anything has rendered. Built then, it has no sections at all, and
    # `buildcards!` short-circuits on `fx.card !== nothing` forever after. The
    # take then reaches for `fxsections_*` and finds nothing:
    # `ArgumentError: collection must be non-empty`, from inside a beat.
    let fx = VE.findslot(scene_clip(), :scene)
        fx === nothing || VE.dropcard!(fx)
        player.shownclip = nothing
    end
    println("warm-up done in ", round(time() - t0, digits = 1), " s — starting the take")
    flush(stdout)
...
function beat!(name, ok, detail = "")
    ...
    println(ok ? "BEAT OK   " : "BEAT FAIL ", name, "  ", detail); flush(stdout)
...
"""
# Progress on STDOUT, flushed. The beats go to stderr through `@info`, which is
# not streamed while the take runs — so a stalled take looked identical to a slow
# one for half an hour. This prints the take clock every second of video.
const LASTTICK = Ref(-1.0)
function tick!(t)
    now_t[] = t
    if floor(t) > LASTTICK[]
        LASTTICK[] = floor(t)
        println("  take ", lpad(round(Int, t), 3), "s   playhead=",
                player.playhead[], "   job=", round(player.jobprogress[], digits=2))
        flush(stdout)
    end
...
events = [
    ...
    # ------------------------------------------------------- the piece
    Lazy(_ -> (caption[] = "Two raytraced scenes: a LEGO world, and the figure on its own track";
    ...
    LeftClick(), Wait(1.2),
    # WAIT ON THE CARD, not on a guess. Selecting the clip builds its cards on the
    # ui queue, and everything below reaches into `player.fxwidgets` for the rows
    # that build registers — `slotkey()` is `first(...)` over them, so an unbuilt
    # card is `ArgumentError: collection must be non-empty` from inside a beat,
    # naming nothing. 1.2 s was enough at 640x1138 and is not at 1920x1080.
    WaitUntil(() -> any(startswith(String(k), "fxsections_")
                        for k in keys(player.fxwidgets)); timeout = 30.0),
...
# `px_per_unit = 2`, not 1. The take is screen content — glyphs, one-pixel
# timeline curves, slider tracks — and at 1 every one of them is a single pixel
# before H.264 sees it. Rendering at 2x and letting the encoder downsample is the
# single biggest thing between this and a crisp projector image. `compression =
# 10` keeps the INTERMEDIATE near-lossless: it is re-encoded below to burn in
# the captions, and the previous take paid generation loss twice for no reason —
# 260 kbps at 1500x950, against the isubd demo's 1008 kbps at 1120x820. NOT `0`:
# Makie maps compression straight to `-crf`, and `-crf 0` makes libx264 produce
# a stream its own pipe writer then dies on — `IOError: write: broken pipe`,
# at any `px_per_unit`. Measured: 0 fails, 10 works.
FakeInteraction.interaction_record((i, t) -> tick!(t), fig, RAW_MP4, events;
                                   fps = 30, px_per_unit = 1, compression = 10)
...
println("take recorded — encoding"); flush(stdout)
run(`$(FFMPEG_jll.ffmpeg()) -y -i $RAW_MP4 -vf $(speedupfilter(ASS, BUSY))
     -c:v libx264 -crf 18 -preset slow -pix_fmt yuv420p $OUT_MP4`)
