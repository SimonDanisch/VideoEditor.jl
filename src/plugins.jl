...
const BUILTIN_KINDS = EffectKind[
    ...
    EffectKind(:restore, "Restore",
        [FxParam(:strength, "Restore"; min = 0.0, max = 1.0, default = 1.0)],
        [:restore_strength],
        nt -> RestoreEffect(Float32(nt.strength)),
        e -> e isa RestoreEffect,
        e -> (strength = Float64(e.strength),)),
    EffectKind(:matte, "Matte",
        [FxParam(:strength, "Matte"; min = 0.0, max = 1.0, default = 1.0),
         FxParam(:feather, "Feather"; min = 0.0, max = 1.0, default = 0.0)],
        [:matte_strength, :matte_feather],
        nt -> MatteEffect(Float32(nt.strength), Float32(nt.feather)),
        e -> e isa MatteEffect,
        e -> (strength = Float64(e.strength), feather = Float64(e.feather))),
